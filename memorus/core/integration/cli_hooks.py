"""CLI hook implementations for Memorus integration.
...
class CLIPreInferenceHook(PreInferenceHook):
    ...
    def _format_xml(results: list[dict]) -> str:
        ...
        """
        lines = ["<memorus-context>"]
        for r in results:
            mem_id = r.get("id", "unknown")
            score = r.get("score", 0.0)
            metadata = r.get("metadata", {})
            mem_type = metadata.get("memorus_knowledge_type", "knowledge") if isinstance(metadata, dict) else "knowledge"
    ...
    def _format_plain(results: list[dict]) -> str:
        ...
        Output:
            [Memorus] User prefers dark mode in all applications.
            [Memorus] When using pytest, always run with -v flag.
        """
        lines = []
        for r in results:
            content = r.get("memory", "")
            lines.append(f"[Memorus] {content}")
        return "\n".join(lines)
