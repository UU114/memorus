...
def _persist_metadata_update(
    mem0: Any,
    memory_id: str,
    content: str,
    merged_meta: dict[str, Any],
) -> None:
    """Write *content* + *merged_meta* back to a mem0 row, preserving metadata.

    mem0 1.0.x ``Memory.update(memory_id, data)`` only accepts content and
    resets the stored payload metadata to ``{}`` (the ``_update_memory``
    ``metadata`` kwarg defaults to ``None`` -> ``{}``), silently wiping every
    ``memorus_*`` field (decay weight, verification TTL/trust_score,
    recall_count, anchors, scope, curator-merged fields).

    To avoid that data loss we drive the underlying ``_update_memory`` API
    directly, passing the merged metadata so it is written into the payload.
    The memory id is preserved. ``_update_memory`` re-derives the core keys
    (``data``/``hash``/``created_at``/``updated_at``) and carries the promoted
    session ids (``user_id``/``agent_id``/``run_id``/``actor_id``/``role``)
    from the existing payload, so only the additional ``memorus_*`` metadata
    needs to be supplied here.

    Falls back to a full delete()+add() row rewrite when the private
    ``_update_memory`` API is unavailable (id is then regenerated by mem0).
    """
    update_fn = getattr(mem0, "_update_memory", None)
    if callable(update_fn):
        # existing_embeddings={} forces _update_memory to embed the new data.
        update_fn(memory_id, content, {}, metadata=dict(merged_meta))
        return

    # Robust fallback: rewrite the whole row. Loses the original id but keeps
    # all metadata, which is the data we must not lose.
    mem0.delete(memory_id)
    mem0.add(content, metadata=dict(merged_meta), infer=False)
...
class Memory:
    ...
    def _merge_update(
        ...
        """Update a bullet's content while preserving its metadata.

        Wired into the ingest pipeline as ``mem0_update_fn`` for curator
        merges. A bare ``mem0.update(id, content)`` would reset the payload
        metadata to ``{}`` (see ``_persist_metadata_update``), so we merge the
        new content into the existing metadata. ``metadata`` (when supplied by
        the caller, e.g. the matched ExistingBullet) seeds the merge; we still
        re-read the live payload so any field the caller did not know about is
        preserved.
        """
        mem0 = self._ensure_mem0()
        merged: dict[str, Any] = {}
        try:
            raw = mem0.get(memory_id)
            if isinstance(raw, dict):
                live_meta = raw.get("metadata", {})
                if isinstance(live_meta, dict):
                    merged.update(live_meta)
        except Exception as exc:
            logger.debug("_merge_update: get(%s) failed: %s", memory_id, exc)
        if isinstance(metadata, dict):
            merged.update(metadata)
        _persist_metadata_update(mem0, memory_id, content, merged)
    ...
    def _init_ace_engines(self) -> None:
        ...
        try:
            ...
            self._ingest_pipeline = IngestPipeline(
                ...
                mem0_update_fn=self._merge_update if self._mem0 else None,
            )
        except Exception as e:
    ...
    def _rows_of(raw: Any) -> list[dict[str, Any]]:
        """Extract the memory rows from a mem0 get_all() result.

        mem0 1.0.x has returned rows under either "memories" or "results"
        across versions; accept both so erasure never silently misses data.
        """
    ...
    def erase_user(self, user_id: str) -> dict[str, Any]:
        """Permanently erase every memory belonging to *user_id*.

        Implements Right-to-Erasure (GDPR Art. 17) for the user-keyed vector
        store: enumerates the subject's bullets and hard-deletes each one,
        including any previously soft-deleted (``deleted_at``-tagged) rows that a
        global reset would otherwise leave behind. Returns a content-free audit
        summary (counts only — no identifiers, no bullet content).
        """
        if not user_id:
            raise ValueError("erase_user requires a non-empty user_id")

        mem0 = self._ensure_mem0()
        rows = self._rows_of(mem0.get_all(user_id=user_id))
        ...
        for row in rows:
            ...
            try:
                ...
                failed += 1
                logger.warning("erase_user: delete(%s) failed: %s", bid, exc)
                continue
            erased += 1
        ...
        logger.info(
            "erase_user: erased=%d soft_deleted_purged=%d failed=%d",
            erased, soft_deleted_purged, failed,
        )
        return {
            "erased": erased,
    ...
    def purge_soft_deleted(self, user_id: str | None = None) -> int:
        """Hard-delete bullets that were only soft-deleted (``deleted_at`` tag).

        The consolidator marks superseded/expired bullets with a ``deleted_at``
        metadata tag rather than removing them, so personal content and its
        embeddings linger in the store indefinitely. This garbage-collects them.
        Pass *user_id* to scope the sweep; ``None`` purges across all users.
        Returns the number of rows removed.
        """
        mem0 = self._ensure_mem0()
        kwargs: dict[str, Any] = {"user_id": user_id} if user_id else {}
        rows = self._rows_of(mem0.get_all(**kwargs))
        ...
        for row in rows:
            ...
            try:
                ...
                logger.warning("purge_soft_deleted: delete(%s) failed: %s", bid, exc)
                continue
            purged += 1

        logger.info("purge_soft_deleted: purged=%d (user_id=%s)", purged, user_id)
        return purged
    ...
    def build_consolidate_adapter(self) -> Any:
        ...
        def _get_bullet(bid: str) -> BulletRecord | None:
            try:
        ...
        def _update_metadata(bid: str, patch: dict[str, Any]) -> None:
            ...
            meta.update(patch)
            try:
                # Persist the merged metadata; a content-only mem0.update would
                # reset the payload metadata to {} (see _persist_metadata_update).
                _persist_metadata_update(
                    mem0, bid, raw_mem.get("memory", ""), meta,
                )
            except Exception as exc:
                logger.warning(
                ...
                if self._config.ace_enabled and self._ingest_pipeline is not None:
                    ...
                    if curator is not None:
                        ...
                        if curate_result.to_merge:
                            # Update existing memory with new content while
                            # preserving its metadata; a bare self.update()
                            # (content-only) would wipe the memorus_* payload.
                            merge_info = curate_result.to_merge[0]
                            try:
                                _existing_meta = getattr(
                                    merge_info.existing, "metadata", None
                                )
                                self._merge_update(
                                    merge_info.existing.bullet_id,
                                    merge_info.candidate.content,
                                    metadata=dict(_existing_meta)
                                    if isinstance(_existing_meta, dict)
                                    else None,
                                )
                                merged += 1
                            except Exception:
    ...
    def run_decay_sweep(
        ...
        for bullet_id, decay_result in sweep_result.details.items():
            ...
            if abs(decay_result.weight - original.current_weight) > 0.0001:
                try:
                    # Read existing payload, merge the new decay weight, and
                    # persist via the metadata-preserving helper so the rest of
                    # the memorus_* metadata is not wiped (see
                    # _persist_metadata_update).
                    ...
                    if isinstance(mem0_mem, dict):
                        ...
                        _persist_metadata_update(
                            mem0, bullet_id, content, existing_meta,
                        )
                    actual_updated += 1
                    logger.debug(
    ...
    def _build_metadata_update_fn(
        ...
        def _update_metadata(
            ...
            meta.update(patch)
            try:
                # Persist the merged metadata (verified_at / verified_status /
                # trust_score) without wiping the rest of the payload. A bare
                # content-only mem0.update would reset metadata to {}
                # (see _persist_metadata_update).
                _persist_metadata_update(
                    mem0, bullet_id, raw.get("memory", ""), meta,
                )
            except Exception as exc:
                logger.debug(
