...
| **离线能力** | 不可用 | **离线优先**（规则/Git 始终离线；ONNX 嵌入需首次联网下载模型，之后离线） |
| **纠正机制** | 手动覆盖/删除 | **自动 Supersede (知识进化)** |
| **隐私脱敏** | 无内置方案 | **多层 PII 过滤器（密钥/令牌/云凭据/邮箱/电话/SSN/信用卡…）+ 默认本地** |
...
### 2. 联邦运行模式
- **Federation Mode**：通过 **ACE Sync Server** 实现实时双向同步、P2P 验证与治理。*（注：P2P 投票/治理目前由 Python 团队层实现；Rust 后端的治理端点尚在规划、未接入。）*
...
- **Privacy** — 多层内置 PII 脱敏（私钥/OAuth/JWT/各云厂商密钥/GitHub PAT/邮箱/电话/SSN/信用卡/DB 连接串等）+ 可插拔自定义正则。
- **ONNX** — 本地嵌入推理（all-MiniLM-L6-v2）。模型首次使用时从 HuggingFace Hub 下载，之后完全离线；隔离/气隙环境可预先将 `model.onnx` + `tokenizer.json` 放入 `~/.memorus/models/` 实现全程离线。
