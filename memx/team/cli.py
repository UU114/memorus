...
def _ensure_team_enabled():
    ...
    try:
        from memx.team.config import load_team_config

        config = load_team_config()
        if not config.enabled:
            return None, (
                "Team features not enabled. "
                "Set 'enabled: true' in team_config.yaml or MEMX_TEAM_ENABLED=true"
            )
        return config, None
...
def team_sync(full: bool, as_json: bool) -> None:
    ...
    try:
        from memx.team.cache_storage import TeamCacheStorage
        from memx.team.sync_client import AceSyncClient
