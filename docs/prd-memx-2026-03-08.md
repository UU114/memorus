# Product Requirements Document: MemX
...
This Product Requirements Document (PRD) defines the functional and non-functional requirements for MemX. It serves as the source of truth for what will be built and provides traceability from requirements through implementation.
...
MemX 是基于 mem0 开源项目的深度改造 Fork，通过叠加 ACE（Adaptive Context Engine）智能层，将 mem0 从"被动存取型"记忆系统进化为"主动蒸馏型"自适应记忆引擎。面向 AI 产品开发者和企业客户，提供自动知识蒸馏、艾宾浩斯衰退遗忘、语义去重、混合三层检索等能力，同时完整保留 mem0 丰富的基础设施生态。
...
**Description:**
定义 Bullet 作为 MemX 的最小知识单元，在 mem0 的 vector store payload metadata 中嵌入 ACE 结构化字段。每个 Bullet 包含 content（≤500 字符）、section（知识分区）、knowledge_type（Method/Trick/Pitfall/Preference/Knowledge）、instructivity_score（0-100）、recall_count、last_recall、decay_weight（0.0-1.0）等字段。
...
**Description:**
确保 MemX 的公开 API 与 mem0 完全兼容。`Memory` 类的 `add()`, `search()`, `get_all()`, `get()`, `update()`, `delete()`, `delete_all()`, `history()`, `reset()` 方法签名不变。ACE 功能通过配置开关和新增可选参数启用，不影响现有调用方式。

**Acceptance Criteria:**
- [ ] 所有 mem0 公开 API 方法签名保持不变
- [ ] 使用 mem0 默认配置时，行为与 mem0 原版一致
- [ ] mem0 现有测试用例在 MemX 上全部通过
...
**Description:**
新增 ONNX Runtime Embedding Provider，支持本地运行 all-MiniLM-L6-v2 模型（384 维向量），实现零网络依赖的 Embedding 能力。模型文件存储在 `~/.memx/models/` 目录，首次使用时自动下载。

**Acceptance Criteria:**
- [ ] `ONNXEmbedding` 类实现 mem0 的 `EmbeddingBase` 接口
- [ ] 默认使用 all-MiniLM-L6-v2 模型，维度 384
- [ ] 模型文件自动下载到 `~/.memx/models/`，支持离线使用
...
**Description:**
提供 CLI 命令行界面，供用户主动管理记忆库。包括：`memx status`（统计信息）、`memx search <query>`（手动检索）、`memx learn <content>`（手动记录）、`memx list`（列出记忆）、`memx forget <id>`（删除记忆）。

**Acceptance Criteria:**
- [ ] 5 个基础命令实现完成
- [ ] `memx status` 显示记忆总数、各 section 分布、各 knowledge_type 分布、平均 decay_weight
- [ ] `memx search` 使用混合检索引擎
- [ ] `memx learn` 经过 Reflector + Curator 处理
...
**Description:**
将 MemX 打包为可安装的 Python 包发布到 PyPI。包名 `memx`，支持 `pip install memx`。包含合理的依赖管理（核心依赖最小化，ONNX/图存储等作为可选依赖）。

**v2.0 扩展**：新增 `memx[team]` 可选依赖分组，包含 Team Memory 所需的额外依赖。

**Acceptance Criteria:**
- [ ] `pip install memx` 成功安装并可运行
- [ ] 最小安装仅包含核心依赖（SQLite、Pydantic 等）
- [ ] 可选依赖分组：`memx[onnx]`、`memx[graph]`、`memx[all]`
- [ ] README 包含快速开始指南
- [ ] 版本号遵循 SemVer
- [ ] **（v2.0）** `memx[team]` 可选依赖分组
...
**Description:**
按解耦架构要求重构包结构，确保 Core 与 Team Layer 边界清晰。Core（`memx/core/`）包含现有所有引擎（Reflector、Generator、Curator、Decay、Storage、Config、Types、Privacy），Team Layer（`memx/team/`）作为可选扩展包独立存在。依赖方向严格单向：`Core ← Team Layer ← Sync Server`，Core 不 import Team Layer 的任何模块。

**Acceptance Criteria:**
- [ ] `memx/core/` 包含所有现有代码，零修改
- [ ] `memx/team/` 作为独立可选包存在
- [ ] `memx/ext/team_bootstrap.py` 负责检测 Team 配置并注入 Team Layer
- [ ] Core 不 import Team Layer 的任何模块（CI 静态检查）
- [ ] `pip install memx`（不含 team extra）运行正常，现有测试 100% 通过
- [ ] 删除 `memx/team/` 目录后，Core 行为与未引入 Team 方案前完全一致
...
**Description:**
MemX 的公开 Python API 必须与 mem0 v1.0.x 完全向后兼容。现有 mem0 用户可以通过更换 import（`from memx import Memory`）无缝迁移，无需修改任何业务代码。

**Acceptance Criteria:**
- [ ] mem0 官方测试套件在 MemX 上 100% 通过
...
**Description:**
建立与 mem0 上游的定期同步机制。ACE 新增代码集中在独立目录（`memx/reflector/`、`memx/curator/`、`memx/decay/`、`memx/integration/`），最小化与上游的冲突面。
...
**Acceptance Criteria:**
- [ ] `from memx import Memory; m = Memory()` 即可使用
...
**Acceptance Criteria:**
- [ ] `pip install memx`（无 team extra）运行正常
- [ ] 删除 `memx/team/` 后所有 Core 功能正常
...
**Description:**
建立 MemX 的数据基础：定义 Bullet 结构化知识单元模型，扩展 mem0 配置系统，确保 mem0 API 兼容性。这是所有后续功能的地基。
...
**EPIC-001 示例故事：**
- As a developer, I want to `pip install memx` and use `from memx import Memory` with my existing mem0 code, so that I can migrate without changing any business logic.
...
**EPIC-002 示例故事：**
- As a MemX user, I want my repeated debugging patterns to be automatically detected and saved, so that the AI remembers my troubleshooting approaches.
...
**EPIC-006 示例故事：**
- As an AI product builder, I want MemX to automatically inject relevant memories before each LLM call, so that my AI assistant gets smarter over time without manual effort.
...
- As a contributor, I want `pip install memx` (without team extra) to work normally, so that I don't need team dependencies for local-only usage.
...
- **需求**: 开箱即用、低成本、本地运行
- **使用场景**: `pip install memx` → 几行代码集成到自己的 AI 产品中
...
- **使用场景**: 评估 MemX Enterprise，在企业 AI 客服系统中集成；**部署 Federation Server 实现团队记忆共享**
...
```
memx status → 查看记忆库统计
    → memx search "async await 错误" → 混合检索
    → memx learn "Rust 中使用 tokio 时需要..." → 手动记录
...
- 端到端加密存储
- **ACE Sync Server 的具体实现**（作为独立项目，不在 MemX 核心库范围内）
...
3. **包名冲突**: PyPI 上 `memx` 是否已被占用？需要提前确认备选名称。
